...
def ext_iv_measured(dev_data_path, device, corners,dev_path):
    ...
    for corner in corners:
        ...
        for i in range(0, loops):
            ...
            idf.to_csv(f"{dev_path}/measured_iv/measured_A{width}_P{length}_t{temp}_{corner}.csv")
            
            leng.append(length)
            wid.append(width)
            tempr.append(temp)
            cor.append(corner)
            meas.append(f"{dev_path}/measured_iv/measured_A{width}_P{length}_t{temp}_{corner}.csv")
...
def run_sim(char,dirpath, device, length, width, corner, temp):
    ...
    netlist_path = f"{dirpath}/{device}_netlists_{char}/netlist_A{width_str}_P{length_str}_t{temp_str}_{corner}.spice"
    result_path = f"{dirpath}/simulated_{char}/simulated_A{width_str}_P{length_str}_t{temp_str}_{corner}.csv"
...
def main():
    ...
    for i, dev in enumerate(devices):
        ...
        for c in char :
            ...
            if diode_file == "" :
                print(f"# No cv_datapoints available for validation for device {dev}")
                continue
            ...
            print(f"# Device {dev} number of {c}_measured_datapoints : ", len(meas_df)*meas_len)
            ...
            merged_out.to_csv(f"{dev_path}/error_analysis_cv.csv", index=False)
